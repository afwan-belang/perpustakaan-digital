<form action="{{ route('home') }}" method="GET" class="relative w-full md:w-80">
    ...
    @if(request('search'))
        <a href="{{ route('home', ['category_id' => request('category_id')]) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors" title="Hapus pencarian">
