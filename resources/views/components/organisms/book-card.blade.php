...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 flex flex-col hover:bg-slate-50 dark:hover:bg-slate-800/80 hover:border-indigo-300 dark:hover:border-slate-700 transition-all duration-300 group relative">
    
    <div class="w-full aspect-[3/4] bg-slate-100 dark:bg-slate-950 rounded-xl mb-5 overflow-hidden shadow-sm dark:shadow-md relative flex items-center justify-center cursor-pointer" onclick="bukaBuku('{{ asset('storage/' . $book->file_pdf) }}')">
        @if($book->cover_image)
    ...
    @auth
        @if(Auth::user()->role !== 'admin')
            @php
                // Mengecek apakah buku ini ada di daftar favorit
                $isFavorite = Auth::user()->favorites->contains($book->id);
            @endphp
            <div class="absolute top-7 right-7 z-10">
                <form action="{{ route('favorites.toggle', $book->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="p-2 rounded-full backdrop-blur-md transition-all shadow-sm {{ $isFavorite ? 'bg-red-500/90 text-white hover:bg-red-600' : 'bg-white/80 dark:bg-slate-800/80 text-slate-400 dark:text-slate-500 hover:text-red-500' }}" title="{{ $isFavorite ? 'Hapus dari Favorit' : 'Tambah ke Favorit' }}">
    ...
    <div class="flex-1 flex flex-col justify-between cursor-pointer" onclick="bukaBuku('{{ asset('storage/' . $book->file_pdf) }}')">
        <div>
