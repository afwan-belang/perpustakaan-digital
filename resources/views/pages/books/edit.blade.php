<x-app-layout>
    <div class="max-w-4xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-white tracking-tight mb-2">Edit Buku</h1>
            <p class="text-slate-400">Perbarui informasi untuk buku: <span class="text-indigo-400 font-semibold">{{ $book->title }}</span></p>
        </div>
            ...
            <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-800">
                    <div>
                        ...
                        />
                        @if($book->cover_image)
                            <p class="text-xs text-slate-400 mt-2">Cover saat ini terpasang. Upload baru untuk mengganti.</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        />
                         @if($book->file_pdf)
                            <p class="text-xs text-slate-400 mt-2">File PDF saat ini terpasang. Upload baru untuk mengganti.</p>
                        @endif
                    </div>
                </div>
